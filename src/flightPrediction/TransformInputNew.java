...
public class TransformInputNew
{
	...
	public static Map<String, Map<Integer, Map<Integer, DelayInfo>>> flightDelayRate = new HashMap<>(); // flightCompany + season + period + delayRate (throughput > 100000)
	...
	public static Map<String,  Rate> weatherCount = new HashMap<>(); // weatherInfo + WeatherRate
	public static Map<String,  Float> weatherRate = new HashMap<>();
	public static Map<String, Rate> planeNoCount = new HashMap<>(); // planeNo + PlaneNoRate
	public static Map<String, Float> planeNoRate = new HashMap<>();
	public static Map<String, Map<Integer, Rate>> flightNoCount = new HashMap<>(); // flightNo + FlightNoRate
	public static Map<String, Map<Integer, Float>> flightNoRate = new HashMap<>();
	public static Map<String, Map<Integer, Rate>> cityCount = new HashMap<>(); 
	public static Map<String, Map<Integer, Float>> cityRate = new HashMap<>(); // city + month + CityRate
	public static Map<String, Map<Integer, Rate>> companyCount = new HashMap<>(); 
	public static Map<String, Map<Integer, Float>> companyRate = new HashMap<>(); // flightCompany + month + CityRate
	//0825
	...
	public static class PlaneNoRate {
		public int planeCount = 0;
		public int planeDelay = 0;
		public Long delayTimes = new Long(0);
		public void statistics(FdInfo fdInfo) {
			planeCount++;
			if (fdInfo.ifDelay == 1) {
				planeDelay++;
	...
	public static class FlightNoRate {
		public int flightCount = 0;
		public int flightDelay = 0;
		public void statistics(FdInfo fdInfo) {
			flightCount++;
			if (fdInfo.ifDelay == 1) {
				flightDelay++;
	...
	public static void splitMaInfo(String splitString, String weather, List<MaInfo> weatherResult)
	{
		if (splitString.equals("早间")) {
			for (String weatherString : weatherList)
			{
				...
				if (weatherString.equals("晴,星光璀灿,月光明亮,阳光,暖")) {
					maInfo.afternoon = 1;
				}
		...
		try
		{
			...
			while (fileInfo.readRecord())
			{
				...
				if (!startTimePlan.isEmpty())
				{
					...
					//0904
					Date startPlan = new Date(fdInfo.startTimePlan * 1000);
					int startHour = Integer.parseInt(dfs4.format(startPlan));
					String startDay = dfs2.format(startPlan);
					...
					if (!landTimePlan.isEmpty())
					{
						...
						Date landPlan = new Date(fdInfo.landTimePlan * 1000);
						int landHour = Integer.parseInt(dfs4.format(landPlan));
						String landDay = dfs2.format(landPlan);
					...
					if (!weatherCount.containsKey(weather)) {
						weatherCount.put(weather, new Rate());
					}
					weatherCount.get(weather).statistics(fdInfo);
					if (!planeNoCount.containsKey(fdInfo.planeNo)) {
						planeNoCount.put(fdInfo.planeNo, new Rate());
					}
					planeNoCount.get(fdInfo.planeNo).statistics(fdInfo);
					if (!flightNoCount.containsKey(fdInfo.flightNo)) {
						flightNoCount.put(fdInfo.flightNo, new HashMap<>());
					}
					Map<Integer, Rate> flightMonth = flightNoCount.get(fdInfo.flightNo);
					if (!flightMonth.containsKey(month)) {
						flightMonth.put(month, new Rate());
					}
					flightMonth.get(month).statistics(fdInfo);
					if (!cityCount.containsKey(fdInfo.startPort)) {
						cityCount.put(fdInfo.startPort, new HashMap<>());
					}
					Map<Integer, Rate> cityMonth = cityCount.get(fdInfo.startPort);
					if (!cityMonth.containsKey(month)) {
						cityMonth.put(month, new Rate());
					}
					cityMonth.get(month).statistics(fdInfo);

					if (!companyCount.containsKey(flightCompany)) {
						companyCount.put(flightCompany, new HashMap<>());
					}
					Map<Integer, Rate> companyMonth = companyCount.get(flightCompany);
					if (!companyMonth.containsKey(month)) {
						companyMonth.put(month, new Rate());
					}
					companyMonth.get(month).statistics(fdInfo);
					//0825
				}
			}
			fileInfo.close();
	...
	public static void processData()
	{
		...
		//0825
		for (Entry<String, Rate> planeNo : planeNoCount.entrySet()) {
			if (planeNo.getValue().count > 10) {
				float rate = (float)planeNo.getValue().delay / (float)planeNo.getValue().count;
				planeNoRate.put(planeNo.getKey(), rate);
			}
		}
		for (Entry<String, Rate> weather : weatherCount.entrySet()) {
		...
		//monthRate(weatherCount, weatherRate);
		monthRate(flightNoCount, flightNoRate);
		monthRate(cityCount, cityRate);
		monthRate(companyCount, companyRate);
	}
	public static void monthRate(Map<String, Map<Integer, Rate>> processMonthCount, Map<String, Map<Integer, Float>> processMonthRate) {
		for (Entry<String, Map<Integer, Rate>> monthCount : processMonthCount.entrySet()) {
			for (Entry<Integer, Rate> month : monthCount.getValue().entrySet()) {
				if (month.getValue().count > 10) {
					float rate = (float)month.getValue().delay / (float)month.getValue().count;
					if (!processMonthRate.containsKey(monthCount.getKey())) {
						processMonthRate.put(monthCount.getKey(), new HashMap<>());
					}
					processMonthRate.get(monthCount.getKey()).put(month.getKey(), rate);
				}
			}
		}
	}
	...
	public static void printData(String fileName) throws IOException
	{
		...
		for (Entry<String, Map<String, List<FdInfo>>> flightInfoEntry : flightInfos.entrySet())
		{
			for (Entry<String, List<FdInfo>> flightDateEntry : flightInfoEntry.getValue().entrySet())
			{
				...
				{
					//0825
					if (fdInfo.startTimePlan < fdInfo.landTimePlan) {
						...
						if (routeWeather.containsKey(date))
						{
							...
							if (routeWeatherPort.containsKey(fdInfo.startPort))
							{
								...
								for (Special special : specialTime) {
									...
									if (affectedStart > 0
											&& endTime - fdInfo.startTimePlan > 3*60*60 && fdInfo.startTimePlan - collectTime > 2 * 60 * 60)
									{
										flag = true;
							...
							if (routeWeatherPort.containsKey(fdInfo.landPort) && fdInfo.landTimePlan != null)
							{
								...
								for (Special special : specialTime) {
									...
									if (affectedStart > 0
											&& fdInfo.landTimePlan+3*60*60 < endTime && fdInfo.startTimePlan - collectTime > 2 * 60 * 60)
									{
										flag = true;
										outputBuffer.write(index + ":" + 1 + " ");
										index++;
...
//						index = 205;
//						if (!fdInfo.planeNo.equals("")) {
//							if (planeNoRate.containsKey(fdInfo.planeNo)) {
//								outputBuffer.write(index + ":" + planeNoRate.get(fdInfo.planeNo) + " ");
//							}
//						}
//						index++;
//						if (!fdInfo.flightNo.equals("")) {
//							if (flightNoRate.containsKey(fdInfo.flightNo)) {
//								if (flightNoRate.get(fdInfo.flightNo).containsKey(month)) {
//									outputBuffer.write(index + ":" + flightNoRate.get(fdInfo.flightNo).get(month) + " ");
//								}
//							}
//						}
//						index++;
//						if (cityRate.containsKey(fdInfo.startPort)) {
//							if (cityRate.get(fdInfo.startPort).containsKey(month)) {
//								outputBuffer.write(index + ":" + cityRate.get(fdInfo.startPort).get(month) + " ");
//							}
//						}
//						index++;
//						if (cityCount.containsKey(fdInfo.startPort)) {
//							if (cityCount.get(fdInfo.startPort).containsKey(month)) {
//								outputBuffer.write(index + ":" + cityCount.get(fdInfo.startPort).get(month).count + " ");
//							}
//						}
//						index++;
//						if (companyRate.containsKey(company)) {
//							if (companyRate.get(company).containsKey(month)) {
//								outputBuffer.write(index + ":" + companyRate.get(company).get(month) + " ");
//							}
//						}
//						index++;
//						if (companyCount.containsKey(company)) {
//							if (companyCount.get(company).containsKey(month)) {
//								outputBuffer.write(index + ":" + companyCount.get(company).get(month).count + " ");
//							}
//						}
//						index++;
						//0825
